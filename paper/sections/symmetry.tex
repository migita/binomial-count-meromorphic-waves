...
In words, \(\iota\) reads a wave in the opposite direction and then
restores the normalization: it moves the zero limit back to the end
\(e^z\to0\), and for odd \(p\) it changes the sign of the profile. The
pairs \(A\) and \(\iota A\) are the mirror images that \(N_p\) counts
separately. For \(p=3\) the involution is \((a,b)\mapsto(-a,b)\) in
Table~\ref{tab:ks}.
...
Thus the rational-exponential waves of a purely dispersive equation are
pulses, and those of a purely dissipative equation are fronts. The
first statement holds in every even order. The second is proved
whenever all pairs are distinct, and for real waves in every odd order.
Elliptic waves occur in both subfamilies and are not concerned.
