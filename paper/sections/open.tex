\section{Open questions and the boundary of the classification}\label{sec:open}

Two questions remain open in the enumeration, and one limitation of
the classification deserves a precise statement.

\paragraph{Rigidity.}
The first question is the continuous-convolution rigidity assertion
\[
 0\ne A\in\CC[\rho],\qquad
 A\mid\int_0^\rho A(t)A(\rho-t)\dd t
 \quad\Longrightarrow\quad A=c\rho^{\deg A}.
\]
Through the polynomial Laplace correspondence used in the proof of
Theorem~\ref{thm:elliptic-count}, it says that, after fixing the pole
at zero, any rational one-pole solution of \eqref{eq:main} vanishing at
infinity would have to be \(c_\ast z^{-p}\), with operator
\(P=D^p\).
Algebraic and \(p\)-adic arguments prove it in the range
\(\deg A\le72\), corresponding to \(2\le p\le73\), and in infinite
arithmetic families of higher degrees.
Rigidity in arbitrary degree remains a conjecture.
By Theorems~\ref{thm:length} and \ref{thm:elliptic-count}, it
would establish both enumeration formulas with algebraic
multiplicities at every order.

\paragraph{Simplicity.}
The second question is whether every point of \(\Xp\), the
rational-exponential matching system, is simple. The elliptic
simplicity assertion is generic in the lattice and is derived from
this one in Theorem~\ref{thm:elliptic-count}.
At a solution \(S_A=AC_A\), the tangent map of the system is
\begin{equation}\label{eq:tangent}
 J_A(h)=\remA\bigl(2(A\star h)-C_Ah\bigr),\qquad \deg h<d.
\end{equation}
The question is whether this map always has zero kernel.
The prime-index argument proves this after reduction modulo
\(\ell\), where the map becomes multiplication by a polynomial
coprime to \(\overline A\); a uniform replacement is missing at
composite indices.
Laurent uniqueness concerns a fixed operator, whereas
\eqref{eq:tangent} allows the operator to vary.
It therefore does not by itself prove simplicity of \(\Xp\).

\begin{conjecture}\label{conj:count}
Continuous-convolution rigidity holds in every degree, and the
matching Jacobian \eqref{eq:tangent} is invertible at every point
of \(\Xp\). Consequently
\(\#\Xp(\CC)=\len\Xp=\binom{2p-1}{p-1}\) for every \(p\ge2\).
\end{conjecture}

The conjecture would also imply \(M_p\) simple elliptic pairs
on a generic fixed lattice at every order, and \(F_p\) distinct pairs
in the symmetric families of Section~\ref{sec:reflection}.
Simplicity on every lattice is false already at order two;
\eqref{eq:ks-collisions} describes the full exceptional set
at order three.

\paragraph{Mixed even operators.}
The even-operator hypothesis in Theorem~\ref{thm:complete} is a
separate issue, and it cannot simply be dropped.
The Fisher equation supplies a known illustration
of the limitation: special-speed solutions were obtained by
\citet{AblowitzZeppetella1979}, and exact solitary representations
were also studied by \citet{KudryashovFisher2005}.
Its elliptic-exponential family is also given in
\citet{Kudryashov2010},
\citet[Lemma~4.4]{ConteNgWu2015}, and
\citet{McCueElHachemSimpson2021}.
In our normalization it is
\[
 v(z)=-12\lambda^2e^{2\lambda z}
       \wp(e^{\lambda z}-1;0,g_3),\qquad
 P(D)=D^2-5\lambda D+6\lambda^2,\qquad\lambda\ne0.
\]
For generic \(g_3\ne0\), this is globally meromorphic and lies
outside \(\Wclass\).
Further examples of meromorphic solutions outside \(\Wclass\)
are discussed by \citet[Remark~1.11]{NgWu2019}.
Substitution follows from
\((D^2-5\lambda D+6\lambda^2)[t^2U(t)]
=\lambda^2t^4U''(t)\), \(t=e^{\lambda z}\).
Thus an unrestricted mixed even-order theorem requires a broader
global conclusion.

The energy argument does have a partial analogue for some mixed
operators, which admit a polynomial Darboux identity
\((D+3\mu)\mathcal J=(v'+\mu v)E_P(v)\).
For identities with \(\mathcal J=v^3/6+\text{quadratic jet terms}\),
the zero level has the same one-pole argument, while nonzero levels
satisfy the nonautonomous relation \(\mathcal J=C e^{-3\mu z}\).
A concrete higher-order test is
\(P=(D+2)(D+3)((D+3)^2+1)\), with marked coefficient
\([z^8]v=0\): the local Laurent condition is compatible and the
Darboux constant is nonzero, as checked in the ancillary material.
This also shows that evenness is sufficient, but not necessary,
for local resonance compatibility.
It is not covered by the even-operator or zero-level argument.
Methods for constructing elliptic and rational-exponential
solutions without the finiteness property are developed by
\citet{Demina2022}.
The global classification of solutions on nonzero Darboux levels
of general mixed even operators is not addressed by the present
argument.
