...
| `label_checks.py` | What the paper reads off the labels, at the prime indices 5 and 7: the labels of all pairs with real coefficients at p = 3 (and of the two complex pairs, for both choices of i modulo 5) and of the 21 real pairs at p = 4; a pair is a pulse iff its label contains 0; the mirror image has label -S; `P_A(0) != 0` at the pulses; and, with symbolic coefficients for p = 3, 4, 5, the identity `R_0 = -A(0)(A(0) + C_A(0))` and the parity splitting of the Jacobian on the fixed subspace of the reflection. |
